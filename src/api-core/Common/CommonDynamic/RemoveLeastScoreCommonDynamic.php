...
<?php /* include_once('Common/CommonDynamic/RemoveLeastScoreCommonDynamic/RemoveLeastScoreCommonDynamicCheckParams.php'); */ ?>
<?php include_once('Common/CommonDynamic/GetCommonDynamic.php'); ?>
<?php include_once('Common/CommonDynamic/RemoveCommonDynamic.php'); ?>
...
<?php
/**********
 * RemoveLeastScoreCommonDynamic
 * @memo: (對於 RemoveLeastScoreCommonDynamic 的描述)
 *        1. 拿到已存在的 orig_obj
 *        2. 用 GetRemoveObj 拿到要去掉的 obj (remove_obj)
 *        3. 如果要去掉的不是 new_obj: Remove CommonDynamic
 ...
 */
function RemoveLeastScoreCommonDynamic($key_id, $length, &$new_obj, $col_start = "", $col_end = "") {
  global $NoSQL;
  $DEBUG_FILENAME = "#" . __FILE__ . "@" . __FUNCTION__;

  if(isset($params['session']) && !isset($NoSQL['SESSION'])) $NoSQL['SESSION'] = $params['session'];

  $cf_common_dynamic = GetCFS('common_dynamic');
  ...
  if(!empty($new_obj)) LengthLimitedRemoveSameColumnName($orig_obj_ary, $new_obj, $cf_common_dynamic, $key_id);
  ...
  foreach($remove_obj_ary as $remove_obj) {
    if($remove_obj !== null && !isset($new_obj[$remove_obj['column_name']])) {
      ...
      $result_remove = RemoveCommonDynamic($key_id, array($remove_obj['column_name']));
