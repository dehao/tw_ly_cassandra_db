...
<?php include_once('Common/CommonDynamic/AddLimitedLengthCommonDynamic/AddLimitedLengthCommonDynamicCheckParams.php'); ?>
<?php include_once('Common/CommonDynamic/ParseNewObjCommonDynamic.php'); ?>
<?php ?>
<?php include_once('Common/CommonDynamic/GetCommonDynamic.php'); ?>
<?php include_once('Common/CommonDynamic/GetCountCommonDynamic.php'); ?>
...
/**********
 * # MEMO
 * 如果 CommonDynamic 本身是有限長度的. 用 AddLimitedLengthCommonDynamic 增加到 CommonDynamic. 
 * 
/// #### GENERAL REQUIRE
...
* 4. 拿到原本的 array. (GetCommonDynamic 目前 policy: 因為長度是有限的. 而且是 add. 所以就全拿. (如果太多的話. 要考慮比較好的縮短長度的方法))
...
*/
function AddLimitedLengthCommonDynamic(&$params, $col_start = "", $col_end = "", $ttl = null, $is_check_params = true) {
  global $NoSQL;
  ...
  if(!isset($params['cols'])) {
    if($is_check_params && $result_check_params = AddLimitedLengthCommonDynamicCheckParams($params, $result)) {
  ...
  $new_obj = ParseNewObjCommonDynamic($params);
  ...
  $data_count = GetCountCommonDynamic($null_ary, $key_id, null, false, 100 * $NoSQL['MAGIC_LENGTH_LIMITED_MAX_LENGTH_FACTOR'], $col_start, $col_end);
  Debug("INFO", __LINE__ . $DEBUG_FILENAME, "after GetCountCommonDynamic: data_count", $data_count);
  $count_common_dynamic = $data_count['error'] ? 0 : $data_count['result'];

  $length_to_remove = $count_common_dynamic + $count_new_obj - 100;
  ...
  Debug("INFO", __LINE__ . $DEBUG_FILENAME, "", "count_common_dynamic: $count_common_dynamic count_new_obj: $count_new_obj length_to_remove: $length_to_remove length_to_get_orig: $length_to_get_orig length_to_preserve: $length_to_preserve");

  $orig_obj = array();
  if($length_to_get_orig > 0) {
    $params_orig = GetCommonDynamic($null_ary, $key_id, null, false, $length_to_get_orig, $col_start, $col_end);
    Debug("INFO", $DEBUG_FILENAME, "after GetCommonDynamic: params_orig", $params_orig);
  ...
  if(!empty($remove_cols) && RemoveData($cf_common_dynamic, $key_id, $remove_cols)) {
    $result['error'] = ErrorCode('COMMON_DYNAMIC', $function_name, 'REMOVE');
    return $result;
  }

  $add_cols =& $cols['add_cols'];
  Debug("INFO", __LINE__ . $DEBUG_FILENAME, "to SetData: key_id: $key_id add_cols", $add_cols);
  if(!empty($add_cols) && SetData($cf_common_dynamic, $key_id, $add_cols, $ttl)) {
