...
<?php /* include_once('Search/Search/RemoveLeastScoreSearch/RemoveLeastScoreSearchCheckParams.php'); */ ?>
<?php include_once('Search/Search/GetSearch.php'); ?>
<?php include_once('Search/Search/RemoveSearch.php'); ?>
...
<?php
/**********
 * RemoveLeastScoreSearch
 * @memo: (對於 RemoveLeastScoreSearch 的描述)
 *        1. 拿到已存在的 orig_obj
 *        2. 用 GetRemoveObj 拿到要去掉的 obj (remove_obj)
 *        3. 如果要去掉的不是 new_obj: Remove Search
 ...
 */
function RemoveLeastScoreSearch($key_id, $length, &$new_obj, $col_start = "", $col_end = "") {
  global $NoSQL;
  $DEBUG_FILENAME = "#" . __FILE__ . "@" . __FUNCTION__;

  if(isset($params['session']) && !isset($NoSQL['SESSION'])) $NoSQL['SESSION'] = $params['session'];

  $cf_search = GetCFS('search');
  ...
  if(!empty($new_obj)) LengthLimitedRemoveSameColumnName($orig_obj_ary, $new_obj, $cf_search, $key_id);
  ...
  foreach($remove_obj_ary as $remove_obj) {
    if($remove_obj !== null && !isset($new_obj[$remove_obj['column_name']])) {
      ...
      $result_remove = RemoveSearch($key_id, array($remove_obj['column_name']));
