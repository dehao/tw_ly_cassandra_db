...
<?php include_once('Search/SearchIndex/MultiAddSearchIndex/MultiAddSearchIndexCheckParams.php'); ?>
<?php include_once('Search/SearchIndex/MultiAddSearchIndex/MultiAddSearchIndexDealWith.php'); ?>
 ...
 * #### More
 * 最單純的 call: MultiAddSearchIndex($params, "", "", null, false, false);
 *
 * # DO
 * 1. 如果沒有 rows 的話:
...
function MultiAddSearchIndex(&$params, $col_start = "", $col_end = "", $ttl = null, $is_check_params = true, $is_deal_with = true) {
  global $NoSQL;
  ...
  if(!isset($params['rows'])) {
    ...
    if($is_check_params && $result_check_params = MultiAddSearchIndexCheckParams($params, $result)) {
  ...
  if(MultiSetData($cf_search_index, $add_rows, $ttl)) {
  ...
  if($is_deal_with && $result_deal_with = MultiAddSearchIndexDealWith($key_ids, $params, $result)) {
