...
<?php /* include_once('Search/SearchIndex/RemoveLeastScoreSearchIndex/RemoveLeastScoreSearchIndexCheckParams.php'); */ ?>
<?php include_once('Search/SearchIndex/GetSearchIndex.php'); ?>
<?php include_once('Search/SearchIndex/RemoveSearchIndex.php'); ?>
...
<?php
/**********
 * RemoveLeastScoreSearchIndex
 * @memo: (對於 RemoveLeastScoreSearchIndex 的描述)
 *        1. 拿到已存在的 orig_obj
 *        2. 用 GetRemoveObj 拿到要去掉的 obj (remove_obj)
 *        3. 如果要去掉的不是 new_obj: Remove SearchIndex
 ...
 */
function RemoveLeastScoreSearchIndex($key_id, $length, &$new_obj, $col_start = "", $col_end = "") {
  global $NoSQL;
  $DEBUG_FILENAME = "#" . __FILE__ . "@" . __FUNCTION__;

  if(isset($params['session']) && !isset($NoSQL['SESSION'])) $NoSQL['SESSION'] = $params['session'];

  $cf_search_index = GetCFS('search_index');
  ...
  if(!empty($new_obj)) LengthLimitedRemoveSameColumnName($orig_obj_ary, $new_obj, $cf_search_index, $key_id);
  ...
  foreach($remove_obj_ary as $remove_obj) {
    if($remove_obj !== null && !isset($new_obj[$remove_obj['column_name']])) {
      ...
      $result_remove = RemoveSearchIndex($key_id, array($remove_obj['column_name']));
