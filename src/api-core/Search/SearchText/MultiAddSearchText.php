...
<?php include_once('Search/SearchText/MultiAddSearchText/MultiAddSearchTextCheckParams.php'); ?>
<?php include_once('Search/SearchText/MultiAddSearchText/MultiAddSearchTextDealWith.php'); ?>
 ...
 * #### More
 * 最單純的 call: MultiAddSearchText($params, "", "", null, false, false);
 *
 * # DO
 * 1. 如果沒有 rows 的話:
...
function MultiAddSearchText(&$params, $col_start = "", $col_end = "", $ttl = null, $is_check_params = true, $is_deal_with = true) {
  global $NoSQL;
  ...
  if(!isset($params['rows'])) {
    ...
    if($is_check_params && $result_check_params = MultiAddSearchTextCheckParams($params, $result)) {
  ...
  if(MultiSetData($cf_search_text, $add_rows, $ttl)) {
  ...
  if($is_deal_with && $result_deal_with = MultiAddSearchTextDealWith($key_ids, $params, $result)) {
