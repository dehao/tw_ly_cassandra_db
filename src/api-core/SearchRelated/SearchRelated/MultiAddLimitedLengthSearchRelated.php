...
<?php include_once('SearchRelated/SearchRelated/MultiAddLimitedLengthSearchRelated/MultiAddLimitedLengthSearchRelatedCheckParams.php'); ?>
<?php include_once('SearchRelated/SearchRelated/ParseNewObjSearchRelated.php'); ?>
<?php ?>
<?php include_once('SearchRelated/SearchRelated/MultiGetSearchRelated.php'); ?>
<?php include_once('SearchRelated/SearchRelated/MultiGetCountSearchRelated.php'); ?>
...
/**********
 * # MEMO
 * 如果 SearchRelated 本身是有限長度的. 用 MultiAddLimitedLengthSearchRelated 將多個 keys 增加到 SearchRelated. 
 *
 * # DO
 * 1. 檢查 params (CheckParams)
 * 2. 設定 info
 * 3. 設定 new_obj. either 是 cols (多個 column_name) . or 是 column_name => column_value.
 * 4. 拿到原本的 array. (MultiGetSearchRelated 目前 policy: 因為長度是有限的. 而且是 add. 所以就全拿. (如果太多的話. 要考慮比較好的縮短長度的方法))
 * 5. 對於每 1 個 key: 
 ...
 */
function MultiAddLimitedLengthSearchRelated(&$params, $col_start = "", $col_end = "", $ttl = null, $is_check_params = true) {
  global $NoSQL;
  ...
  if($is_check_params && $result_check_params = MultiAddLimitedLengthSearchRelatedCheckParams($params, $result)) {
  ...
  $data_count = MultiGetCountSearchRelated($null_ary, $key_ids, null, false, 100 * $NoSQL['MAGIC_LENGTH_LIMITED_MAX_LENGTH_FACTOR'], $col_start, $col_end);
  $count_search_related = array();
  $max_count = 0;
  if($data_count['error']) {
    foreach($key_ids as $each_key) $count_search_related[$each_key] = 0;
  }
  else {
    foreach($key_ids as $each_key) {
      $count_search_related[$each_key] = isset($data_count['result'][$each_key]) ? $data_count['result'][$each_key] : 0;
      $max_count = max($max_count, $count_search_related[$each_key]);
    }
  }
  ...
  foreach($key_ids as $each_key) {
    $length_to_remove[$each_key] = $count_search_related[$each_key] + $count_new_obj - 100;
  ...
  if($max_length_to_get_orig > 0) {
    $params_orig = MultiGetSearchRelated($null_ary, $key_ids, null, false, $max_length_to_get_orig, $col_start, $col_end);
  ...
  foreach($key_ids as $each_key) {
    ...
    if(!empty($remove_cols) && RemoveData($cf_search_related, $each_key, $remove_cols)) {
  ...
  if(!empty($add_rows) && MultiSetData($cf_search_related, $add_rows, $ttl)) {
