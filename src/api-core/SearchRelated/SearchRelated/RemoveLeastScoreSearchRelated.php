...
<?php /* include_once('SearchRelated/SearchRelated/RemoveLeastScoreSearchRelated/RemoveLeastScoreSearchRelatedCheckParams.php'); */ ?>
<?php include_once('SearchRelated/SearchRelated/GetSearchRelated.php'); ?>
<?php include_once('SearchRelated/SearchRelated/RemoveSearchRelated.php'); ?>
...
<?php
/**********
 * RemoveLeastScoreSearchRelated
 * @memo: (對於 RemoveLeastScoreSearchRelated 的描述)
 *        1. 拿到已存在的 orig_obj
 *        2. 用 GetRemoveObj 拿到要去掉的 obj (remove_obj)
 *        3. 如果要去掉的不是 new_obj: Remove SearchRelated
 ...
 */
function RemoveLeastScoreSearchRelated($key_id, $length, &$new_obj, $col_start = "", $col_end = "") {
  global $NoSQL;
  $DEBUG_FILENAME = "#" . __FILE__ . "@" . __FUNCTION__;

  if(isset($params['session']) && !isset($NoSQL['SESSION'])) $NoSQL['SESSION'] = $params['session'];

  $cf_search_related = GetCFS('search_related');
  ...
  if(!empty($new_obj)) LengthLimitedRemoveSameColumnName($orig_obj_ary, $new_obj, $cf_search_related, $key_id);
  ...
  foreach($remove_obj_ary as $remove_obj) {
    if($remove_obj !== null && !isset($new_obj[$remove_obj['column_name']])) {
      ...
      $result_remove = RemoveSearchRelated($key_id, array($remove_obj['column_name']));
