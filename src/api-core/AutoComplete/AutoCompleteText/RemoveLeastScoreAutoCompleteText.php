...
<?php /* include_once('AutoComplete/AutoCompleteText/RemoveLeastScoreAutoCompleteText/RemoveLeastScoreAutoCompleteTextCheckParams.php'); */ ?>
<?php include_once('AutoComplete/AutoCompleteText/GetAutoCompleteText.php'); ?>
<?php include_once('AutoComplete/AutoCompleteText/RemoveAutoCompleteText.php'); ?>
...
<?php
/**********
 * RemoveLeastScoreAutoCompleteText
 * @memo: (對於 RemoveLeastScoreAutoCompleteText 的描述)
 *        1. 拿到已存在的 orig_obj
 *        2. 用 GetRemoveObj 拿到要去掉的 obj (remove_obj)
 *        3. 如果要去掉的不是 new_obj: Remove AutoCompleteText
 ...
 */
function RemoveLeastScoreAutoCompleteText($key_id, $length, &$new_obj, $col_start = "", $col_end = "") {
  global $NoSQL;
  $DEBUG_FILENAME = "#" . __FILE__ . "@" . __FUNCTION__;

  if(isset($params['session']) && !isset($NoSQL['SESSION'])) $NoSQL['SESSION'] = $params['session'];

  $cf_auto_complete_text = GetCFS('auto_complete_text');
  ...
  if(!empty($new_obj)) LengthLimitedRemoveSameColumnName($orig_obj_ary, $new_obj, $cf_auto_complete_text, $key_id);
  ...
  foreach($remove_obj_ary as $remove_obj) {
    if($remove_obj !== null && !isset($new_obj[$remove_obj['column_name']])) {
      ...
      $result_remove = RemoveAutoCompleteText($key_id, array($remove_obj['column_name']));
