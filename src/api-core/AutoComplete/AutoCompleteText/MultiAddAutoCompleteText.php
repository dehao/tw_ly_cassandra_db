...
<?php include_once('AutoComplete/AutoCompleteText/MultiAddAutoCompleteText/MultiAddAutoCompleteTextCheckParams.php'); ?>
<?php include_once('AutoComplete/AutoCompleteText/MultiAddAutoCompleteText/MultiAddAutoCompleteTextDealWith.php'); ?>
 ...
 * #### More
 * 最單純的 call: MultiAddAutoCompleteText($params, "", "", null, false, false);
 *
 * # DO
 * 1. 如果沒有 rows 的話:
...
function MultiAddAutoCompleteText(&$params, $col_start = "", $col_end = "", $ttl = null, $is_check_params = true, $is_deal_with = true) {
  global $NoSQL;
  ...
  if(!isset($params['rows'])) {
    ...
    if($is_check_params && $result_check_params = MultiAddAutoCompleteTextCheckParams($params, $result)) {
  ...
  if(MultiSetData($cf_auto_complete_text, $add_rows, $ttl)) {
  ...
  if($is_deal_with && $result_deal_with = MultiAddAutoCompleteTextDealWith($key_ids, $params, $result)) {
