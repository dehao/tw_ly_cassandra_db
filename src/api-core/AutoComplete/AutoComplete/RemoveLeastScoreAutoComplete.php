...
<?php /* include_once('AutoComplete/AutoComplete/RemoveLeastScoreAutoComplete/RemoveLeastScoreAutoCompleteCheckParams.php'); */ ?>
<?php include_once('AutoComplete/AutoComplete/GetAutoComplete.php'); ?>
<?php include_once('AutoComplete/AutoComplete/RemoveAutoComplete.php'); ?>
...
<?php
/**********
 * RemoveLeastScoreAutoComplete
 * @memo: (對於 RemoveLeastScoreAutoComplete 的描述)
 *        1. 拿到已存在的 orig_obj
 *        2. 用 GetRemoveObj 拿到要去掉的 obj (remove_obj)
 *        3. 如果要去掉的不是 new_obj: Remove AutoComplete
 ...
 */
function RemoveLeastScoreAutoComplete($key_id, $length, &$new_obj, $col_start = "", $col_end = "") {
  global $NoSQL;
  $DEBUG_FILENAME = "#" . __FILE__ . "@" . __FUNCTION__;

  if(isset($params['session']) && !isset($NoSQL['SESSION'])) $NoSQL['SESSION'] = $params['session'];

  $cf_auto_complete = GetCFS('auto_complete');
  ...
  if(!empty($new_obj)) LengthLimitedRemoveSameColumnName($orig_obj_ary, $new_obj, $cf_auto_complete, $key_id);
  ...
  foreach($remove_obj_ary as $remove_obj) {
    if($remove_obj !== null && !isset($new_obj[$remove_obj['column_name']])) {
      ...
      $result_remove = RemoveAutoComplete($key_id, array($remove_obj['column_name']));
