...
/// @param [=in] $params['cols'] = [not_isset]                        <ul><li>type: string</li><li>[中文(傳統)] 以 array(column_name_id => column_value_id) 表示的 cols. (column_name_id 和 column_value_id 可以用 SerializeColumnNameCommonDynamic 和 SerializeColumnValueCommonDynamic 取得) (適用於對於同 1 個 key add 很多個 cols.)</li></ul>
...
/// @param [=out] column_name_id                                      <ul><li>type: string</li><li>[中文(傳統)] column_name_id</li></ul>
/// 
/// # DO
/// 1. 如果有設定 cols 的話: 
/// 1.1. 設定 key_id.
...
/// # DealWith
//////////
function AddCommonDynamic(&$params, $ttl = null, $is_check_params = true, $is_deal_with = true) {
  global $NoSQL;
  ...
  if(isset($params['cols'])) {
    ...
    //1.2
    if($is_check_params && $result_check_params = AddCommonDynamicCheckParams($params, $result)) {
