...
/// #### More<br />
/// 最單純的 call: MultiAddSearch($params, "", "", null, false, false);<br />
/// <br />
///
/// 
...
/// 
/// # Params
/// @param [(0)!in] &$params                                          <ul><li>type: hash</li><li>[中文(傳統)] 整理好的要做 update 的 rows. 以 array(key => array(column_name => column_value)) 表示. 請參考 search 裡的 key/column_name/column_value</li><li>see: dynamic:: Search.php</li></ul>
...
/// # DealWith
//////////
function MultiAddSearch(&$params, $col_start = "", $col_end = "", $ttl = null, $is_check_params = true, $is_deal_with = true) {
  global $NoSQL;
  ...
  if(!isset($params['rows'])) {
    ...
    if($is_check_params && $result_check_params = MultiAddSearchCheckParams($params, $result)) {
  ...
  if(MultiSetData($cf_search, $add_rows, $ttl)) {
  ...
  if($is_deal_with && $result_deal_with = MultiAddSearchDealWith($key_ids, $params, $result)) {
