...
//////////
/// # MEMO
/// 拿到 Search (require: params['key_id'])<br />
/// <br />
///
/// 
...
/// @param [=in] $params['cf'] = unset                                <ul><li>type: string</li><li>[中文(傳統)] 是否其實是要從其他 cf 裡面拿資料 (search 不用傳).</li></ul>
...
/// # CheckParams
//////////
function GetSearch(&$params, $key_id, $column_names = null, $is_reverse = true, $max_length = LENGTH_GET_DATA, $col_start = "", $col_end = "", $is_with_start = false, $is_check_available = true) {
  global $NoSQL;
  ...
  if($result_check_params = GetSearchCheckParams($params, $key_id, $column_names, $is_reverse, $max_length, $col_start, $col_end, $is_with_start, $is_check_available, $result)) {
  ...
  $data = GetData($cf_search, $key_id, $column_names, $is_reverse, $max_length, $col_start, $col_end);
  Debug("INFO", __LINE__ . $DEBUG_FILENAME, "after GetData: data", $data);
  if($data === null) {
    //$result['error'] = ErrorCode('SEARCH', $function_name, 'GET');
    //return $result;
    $result['result'] = array();
    return $result;
  }
