...
/// @param [!in] $params['search_ids']                                <ul><li>type: string</li><li>[中文(傳統)] 以 array(search_id) 表示的 cols. (適用於對於同 1 個 key add 很多個 search_ids.)</li></ul>
...
/// # DealWith
//////////
function AddMultiColsSearch(&$params, $ttl = null, $is_check_params = true, $is_deal_with = true) {
  global $NoSQL;
  ...
  if(!isset($params['search_ids'])) {
    $result['error'] = ErrorCode('SEARCH', $function_name, 'API1_NO_IDS');
    return $result;
  }
  ...
  foreach($params['search_ids'] as $each_val) {
    $params['search_id'] = $each_val;
    if($is_check_params && $result_check_params = AddSearchCheckParams($params)) {
