...
//////////
/// # MEMO
/// 如果 SearchIndex 本身是有限長度的. 用 AddLimitedLengthSearchIndex 增加到 SearchIndex. <br />
/// <br />
/// #### GENERAL REQUIRE<br />
/// 1. params['key_id']/(表示 key 的 id (請參考 dynamic::search_index))<br />
/// 2. 除非有在 dynamic::search_index 特別說明. 否則 column_name/column_value 裡的 structure 都需要設定. <br />
...
/// 
/// # Params
/// @param [(0)!in] &$params                                          <ul><li>type: hash</li><li>[中文(傳統)] 請參考 dynamic::search_index</li><li>see: dynamic:: SearchIndex.php</li></ul>
/// @param [!in] $params['setter_id']                                 <ul><li>type: string</li><li>[中文(傳統)] $params['setter id']</li><li>see: fixed:: SearchIndex.php</li></ul>
...
/// @param [(1)=in] $col_start = ""                                   <ul><li>type: string</li><li>[中文(傳統)] 在跟已經存在 search_index 裡的 column_name 比較(要刪除)的時候. 要從哪裡開始比 (區間裡小的那個)</li></ul>
/// @param [(2)=in] $col_end = ""                                     <ul><li>type: string</li><li>[中文(傳統)] 在跟已經存在 search_index 裡的 column_name 比較(要刪除)的時候, 比較的結束點 (區間裡大的那個)</li></ul>
...
/// # CheckParams
//////////
function AddLimitedLengthSearchIndex(&$params, $col_start = "", $col_end = "", $ttl = null, $is_check_params = true) {
  global $NoSQL;
  ...
  if(!isset($params['cols'])) {
    if($is_check_params && $result_check_params = AddLimitedLengthSearchIndexCheckParams($params, $result)) {
  ...
  if(isset($params['key_id'])) $key_id = $params['key_id']; else $key_id = SerializeKeySearchIndex($params);
  $params['key_id'] = $key_id;

  $new_obj = ParseNewObjSearchIndex($params);
  ...
  $data_count = GetCountSearchIndex($null_ary, $key_id, null, false, 100 * $NoSQL['MAGIC_LENGTH_LIMITED_MAX_LENGTH_FACTOR'], $col_start, $col_end);
  Debug("INFO", __LINE__ . $DEBUG_FILENAME, "after GetCountSearchIndex: data_count", $data_count);
  $count_search_index = $data_count['error'] ? 0 : $data_count['result'];

  $length_to_remove = $count_search_index + $count_new_obj - 100;
  ...
  Debug("INFO", __LINE__ . $DEBUG_FILENAME, "", "count_search_index: $count_search_index count_new_obj: $count_new_obj length_to_remove: $length_to_remove length_to_get_orig: $length_to_get_orig length_to_preserve: $length_to_preserve");

  $orig_obj = array();
  if($length_to_get_orig > 0) {
    $params_orig = GetSearchIndex($null_ary, $key_id, null, false, $length_to_get_orig, $col_start, $col_end);
    Debug("INFO", $DEBUG_FILENAME, "after GetSearchIndex: params_orig", $params_orig);
  ...
  if(!empty($remove_cols) && RemoveData($cf_search_index, $key_id, $remove_cols)) {
    $result['error'] = ErrorCode('SEARCH_INDEX', $function_name, 'REMOVE');
    return $result;
  }

  $add_cols =& $cols['add_cols'];
  Debug("INFO", __LINE__ . $DEBUG_FILENAME, "to SetData: key_id: $key_id add_cols", $add_cols);
  if(!empty($add_cols) && SetData($cf_search_index, $key_id, $add_cols, $ttl)) {
    $result['error'] = ErrorCode('SEARCH_INDEX', $function_name, 'CF1');
    return $result;
  }
