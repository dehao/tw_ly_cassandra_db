...
//////////
/// # MEMO
/// 如果 AutoCompleteText 本身是有限長度的. 用 AddLimitedLengthAutoCompleteText 增加到 AutoCompleteText. <br />
/// <br />
/// #### GENERAL REQUIRE<br />
/// 1. params['key_id']/(表示 key 的 id (請參考 dynamic::auto_complete_text))<br />
/// 2. 除非有在 dynamic::auto_complete_text 特別說明. 否則 column_name/column_value 裡的 structure 都需要設定. <br />
...
/// 
/// # Params
/// @param [(0)!in] &$params                                          <ul><li>type: hash</li><li>[中文(傳統)] 請參考 dynamic::auto_complete_text</li><li>see: dynamic:: AutoCompleteText.php</li></ul>
/// @param [!in] $params['setter_id']                                 <ul><li>type: string</li><li>[中文(傳統)] $params['setter id']</li><li>see: fixed:: AutoCompleteText.php</li></ul>
/// @param [!in] $params['key_id']                                    <ul><li>type: string</li><li>[中文(傳統)] key_id</li></ul>
/// @param [=in] $params['session'] = [not_isset]                     <ul><li>type: string</li><li>[中文(傳統)] $params['session'] = [not isset]</li><li>see: fixed:: AutoCompleteText.php</li></ul>
/// @param [(1)=in] $col_start = ""                                   <ul><li>type: string</li><li>[中文(傳統)] 在跟已經存在 auto_complete_text 裡的 column_name 比較(要刪除)的時候. 要從哪裡開始比 (區間裡小的那個)</li></ul>
/// @param [(2)=in] $col_end = ""                                     <ul><li>type: string</li><li>[中文(傳統)] 在跟已經存在 auto_complete_text 裡的 column_name 比較(要刪除)的時候, 比較的結束點 (區間裡大的那個)</li></ul>
...
/// # CheckParams
//////////
function AddLimitedLengthAutoCompleteText(&$params, $col_start = "", $col_end = "", $ttl = null, $is_check_params = true) {
  global $NoSQL;
  ...
  if(!isset($params['cols'])) {
    if($is_check_params && $result_check_params = AddLimitedLengthAutoCompleteTextCheckParams($params, $result)) {
  ...
  $new_obj = ParseNewObjAutoCompleteText($params);
  ...
  $data_count = GetCountAutoCompleteText($null_ary, $key_id, null, false, 100 * $NoSQL['MAGIC_LENGTH_LIMITED_MAX_LENGTH_FACTOR'], $col_start, $col_end);
  Debug("INFO", __LINE__ . $DEBUG_FILENAME, "after GetCountAutoCompleteText: data_count", $data_count);
  $count_auto_complete_text = $data_count['error'] ? 0 : $data_count['result'];

  $length_to_remove = $count_auto_complete_text + $count_new_obj - 100;
  ...
  Debug("INFO", __LINE__ . $DEBUG_FILENAME, "", "count_auto_complete_text: $count_auto_complete_text count_new_obj: $count_new_obj length_to_remove: $length_to_remove length_to_get_orig: $length_to_get_orig length_to_preserve: $length_to_preserve");

  $orig_obj = array();
  if($length_to_get_orig > 0) {
    $params_orig = GetAutoCompleteText($null_ary, $key_id, null, false, $length_to_get_orig, $col_start, $col_end);
    Debug("INFO", $DEBUG_FILENAME, "after GetAutoCompleteText: params_orig", $params_orig);
  ...
  if(!empty($remove_cols) && RemoveData($cf_auto_complete_text, $key_id, $remove_cols)) {
  ...
  if(!empty($add_cols) && SetData($cf_auto_complete_text, $key_id, $add_cols, $ttl)) {
