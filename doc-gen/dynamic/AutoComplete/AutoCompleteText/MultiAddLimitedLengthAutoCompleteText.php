...
//////////
/// # MEMO
/// 如果 AutoCompleteText 本身是有限長度的. 用 MultiAddLimitedLengthAutoCompleteText 將多個 keys 增加到 AutoCompleteText. <br />
/// <br />
///
/// 
...
/// 4. 拿到原本的 array. (MultiGetAutoCompleteText 目前 policy: 因為長度是有限的. 而且是 add. 所以就全拿. (如果太多的話. 要考慮比較好的縮短長度的方法))
/// 5. 對於每 1 個 key: 
...
/// # CheckParams
//////////
function MultiAddLimitedLengthAutoCompleteText(&$params, $col_start = "", $col_end = "", $ttl = null, $is_check_params = true) {
  global $NoSQL;
  ...
  if($is_check_params && $result_check_params = MultiAddLimitedLengthAutoCompleteTextCheckParams($params, $result)) {
  ...
  $data_count = MultiGetCountAutoCompleteText($null_ary, $key_ids, null, false, 100 * $NoSQL['MAGIC_LENGTH_LIMITED_MAX_LENGTH_FACTOR'], $col_start, $col_end);
  $count_auto_complete_text = array();
  $max_count = 0;
  if($data_count['error']) {
    foreach($key_ids as $each_key) $count_auto_complete_text[$each_key] = 0;
  }
  else {
    foreach($key_ids as $each_key) {
      $count_auto_complete_text[$each_key] = isset($data_count['result'][$each_key]) ? $data_count['result'][$each_key] : 0;
      $max_count = max($max_count, $count_auto_complete_text[$each_key]);
    }
  }
  ...
  foreach($key_ids as $each_key) {
    $length_to_remove[$each_key] = $count_auto_complete_text[$each_key] + $count_new_obj - 100;
  ...
  if($max_length_to_get_orig > 0) {
    $params_orig = MultiGetAutoCompleteText($null_ary, $key_ids, null, false, $max_length_to_get_orig, $col_start, $col_end);
  ...
  foreach($key_ids as $each_key) {
    ...
    if(!empty($remove_cols) && RemoveData($cf_auto_complete_text, $each_key, $remove_cols)) {
  ...
  if(!empty($add_rows) && MultiSetData($cf_auto_complete_text, $add_rows, $ttl)) {
    $result['error'] = ErrorCode('AUTO_COMPLETE_TEXT', $function_name, 'MULTISET');
    return $result;
  }
