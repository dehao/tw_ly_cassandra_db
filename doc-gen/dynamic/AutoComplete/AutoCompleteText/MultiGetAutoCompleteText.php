...
/// 
//////////
function MultiGetAutoCompleteText(&$params, &$key_ids, $column_names = null, $is_reverse = true, $max_length = LENGTH_GET_DATA, $col_start = "", $col_end = "", $is_with_start = false) {
  global $NoSQL;
   ...
   */
  $data = MultiGetData($cf_auto_complete_text, $key_ids, $column_names, $is_reverse, $max_length, $col_start, $col_end);
  Debug("INFO", __LINE__ . $DEBUG_FILENAME, "after MultiGetData: data", $data);
  if($data === null) {
    //$result['error'] = ErrorCode('AUTO_COMPLETE_TEXT', $function_name, 'MULTIGET');
    //return $result;
    $result['result'] = array();
    return $result;
  }
