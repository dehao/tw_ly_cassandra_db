...
/// #### More<br />
/// 最單純的 call: MultiAddAutoComplete($params, "", "", null, false, false);<br />
/// <br />
///
/// 
...
/// 
/// # Params
/// @param [(0)!in] &$params                                          <ul><li>type: hash</li><li>[中文(傳統)] 整理好的要做 update 的 rows. 以 array(key => array(column_name => column_value)) 表示. 請參考 auto_complete 裡的 key/column_name/column_value</li><li>see: dynamic:: AutoComplete.php</li></ul>
...
/// # DealWith
//////////
function MultiAddAutoComplete(&$params, $col_start = "", $col_end = "", $ttl = null, $is_check_params = true, $is_deal_with = true) {
  global $NoSQL;
  ...
  if(!isset($params['rows'])) {
    ...
    if($is_check_params && $result_check_params = MultiAddAutoCompleteCheckParams($params, $result)) {
  ...
  if(MultiSetData($cf_auto_complete, $add_rows, $ttl)) {
  ...
  if($is_deal_with && $result_deal_with = MultiAddAutoCompleteDealWith($key_ids, $params, $result)) {
